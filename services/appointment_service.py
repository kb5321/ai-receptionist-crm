...
from services.client_service import update_client_from_appointment
...
def get_admin_appointments_page(request):
...
def send_appointment_confirmation_sms(appointment_id: int):
    ...
    cur.execute(
        """
        SELECT client_id, client_name, phone, service, appointment_time
        FROM appointments
        WHERE id = %s
        """,
        (appointment_id,)
